...
from .utils import server_utils, application_utils
...
class ModuleRequestHandler(BaseHTTPRequestHandler):
    ...
    def _execute_handler(self, method, endpoint, pay_load=None):
        ...
        if not is_method_valid:
            ...
            return application_utils.handle_exception(501, WebServerException.__name__, error_msg)

        is_endpoint_valid = self.handlers[method].get(endpoint)

        if not is_endpoint_valid:

            error_msg = f"'{endpoint}' endpoint doesnt support '{method}' method."
            return application_utils.handle_exception(404, WebServerException.__name__, error_msg)
        ...
        if not isinstance(result, (tuple, list)) or len(result) != 3:
            return application_utils.handle_exception(
                500, WebServerException.__name__, f"Invalid handler response from {endpoint}"
            )
    ...
    def _do(self, method):
        ...
        pay_load = self._get_pay_load() if method in CONSTANTS.PAYLOAD_REQUIRED_METHODS else None

        try:
            status, content_type, content = self._execute_handler(method, endpoint, pay_load)
